...
function Personnage() {
    ...
    this.Attaquer = function (Def) {
        ...
        if (0 < this.vie <= 30) {
            ...
            console.log("%cLe combattant " + TableauWinners[Defenseur].nom + " est mort !!", "color: yellow");
            //document.getElementById[TableauWinners].deleteRow(Defenseur);
            console.log(TableauWinners);
            alert("Victoire de " + TableauWinners[Attaquant].nom);
        } else if (this.vie <= 0) {
            console.log("%cLe combattant " + TableauWinners[Attaquant].nom + " est mort !!", "color: yellow");
            //document.getElementById[TableauWinners].deleteRow(Attaquant);
            console.log(TableauWinners);
            alert("Victoire de " + TableauWinners[Defenseur].nom);
        }
    };
};
...
//boucle de combat
function Combat() {
    while (TableauWinners.length > 1) {
        Attaquant = Math.floor(Math.random() * TableauWinners.length);
        Defenseur = Math.floor(Math.random() * TableauWinners.length);
        ...
        if ((Attaquant != OldAttaquant) && (Attaquant != Defenseur)) {
            ...
            if ((TableauWinners[Attaquant].vie > 0) && (TableauWinners[Defenseur].vie > 0)) {
                    console.log(TableauWinners[Attaquant].Attaquer(TableauWinners[Defenseur]));
                } 
        };
        console.log(TableauWinners);
    } console.log(TableauWinners[Attaquant]);
};
